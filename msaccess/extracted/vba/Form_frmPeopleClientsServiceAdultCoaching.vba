' Module Name: Form_frmPeopleClientsServiceAdultCoaching
...
Private Sub Form_Current()
    Call ProgressMessages("Append", "   Opening adult coaching form.")
    If IsNull(CityTown) Then
        CityTown = "Dedham"
        Location = "Adult Coaching"
    End If

    If Inactive Then
        Me.Caption = "Client: Adult Coaching (INACTIVE)"
    Else
        Me.Caption = "Client: Adult Coaching"
...
Private Sub Inactive_Click()
    If Inactive Then
        Call CheckInactive(DateInactive, "AdultCoachSetInactive", "True")
        Call GreyAndNormal(Form_frmPeople.IsClientAdultCoachLabel)
        Form_frmPeople.NumberOfClientServices = Form_frmPeople.NumberOfClientServices - 1
        If Form_frmPeople.NumberOfClientServices < 0 Then Form_frmPeople.NumberOfClientServices = 0
    Else
        Call CheckInactive(DateInactive, "AdultCoachSetInactive", "False")
        Call BlueAndBold(Form_frmPeople.IsClientAdultCoachLabel)
        Form_frmPeople.NumberOfClientServices = Form_frmPeople.NumberOfClientServices + 1
    End If
End Sub

Private Sub SelectBroker_Click()
    DoCmd.OpenForm "frmPeopleSelectPerson", , , , , , "AdultCoachCaseManager"
End Sub
...
Private Sub ContractNumber_AfterUpdate()
    ...
    Call UpdateChangeLog("AdultCoachContractNumber", ContractNumber)
    Call UpdateChangeLog("AdultCoachActivityCode", ActivityCode)
End Sub
